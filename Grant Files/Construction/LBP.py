...
def linear_back_projection(measurements, sensitivity_matrix):
    """
    ...
    reconstructed_image = np.dot(sensitivity_matrix.T, measurements)
...
if __name__ == "__main__":
    ...
    electrode_positions = PhantomGenerator(electrode_count, domain_radius, grid_size).electrodes
    sensitivity_matrix = calculate_sensitivity_matrix(grid_size, electrode_positions, permittivity)

    # Debug: Check sensitivity matrix
    print("Sensitivity matrix stats:", sensitivity_matrix.min(), sensitivity_matrix.max())
