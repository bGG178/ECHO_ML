...
if __name__ == "__main__":
    ...
    target_loss = 0.45
    epochs = 1800
    scheduler_step_size = 350  # Step size for the learning rate scheduler
 ...
 #       print("Compiling...")
    ...
    # Training loop
    for epoch in trange(epochs + 1):  # Assuming epochs is defined
        ...
        if epoch % 4 == 0:
            print(
                ...
                f"If {lambda_L1 * l1_reconstruction_loss} is vastly larger than {adversarial_g_loss}, consider adjusting lambda_L1 to be lower. If the reverse, L1 needs to be larger."
            )
        ...
        if epoch % 80 == 0:
            with torch.no_grad():
                ...
                plt.tight_layout()
                # Consider plt.savefig(f"epoch_{epoch}.png") and plt.close(fig) if running non-interactively
    ...
    modeloutput = r"C:\Users\bower\PycharmProjects\ECHO_ML\Grant Files\SavedModels"
    os.makedirs(modeloutput, exist_ok=True)
    output_file = os.path.join(modeloutput, f"{electrode_count}e{emitting_count}_ECHO_0.2.3.1_model.pth")
    if not os.path.exists(datafile):
        raise FileNotFoundError(f"File not found: {datafile}")
    ...
    with torch.no_grad():
        for i, (real_imgs, cond_input) in enumerate(test_dataloader):
            ...
            # Mass loss
            fakeimgmass = MassCalculator_circular(fake_imgs)
            realimgmass = MassCalculator_circular(real_imgs)
            mloss = (abs(float(fakeimgmass) - float(realimgmass)) / float(realimgmass)) * 100
            with torch.no_grad():
                ic = imagecorrelation_circular(cutoff(real_imgs[0].cpu().numpy(),0.5,tensorreturn=False), cutoff(fake_imgs[0].cpu().numpy(),0.5,tensorreturn=False))
            ...
            with torch.no_grad():
                ...
                for i in range(8):
                    ...
                    mlossgraph = (abs(float(MassCalculator_circular(fake_imgs[i, 0])) - float(
                        MassCalculator_circular(real_imgs[i, 0]))) / float(MassCalculator_circular(real_imgs[i, 0]))) * 100
