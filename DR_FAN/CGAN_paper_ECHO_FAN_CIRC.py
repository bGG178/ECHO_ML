...
import numpy as np
import modulator_FAN_CIRC as m2
import time
...
if __name__ == "__main__":
    ...
    overviewsavelocation = "Outputs/Circle/TrainingOverview__npgCircles10000.png"
 ...
 #       print("Compiling...")
    ...
    for epoch in trange(epochs + 1):
        ...
        if epoch % 100 == 0:
            with torch.no_grad():
                ...
                plt.tight_layout()
                plt.savefig(f"Outputs/Circle/CircleExamples/epoch_{epoch}.png",dpi=300)
                plt.close(fig)
    ...
    output_file = (f"Outputs/Circle/{electrode_count}e{emitting_count}_ECHO_FAN_CIRCLE_model.pth")
    if not os.path.exists(datafile):
        raise FileNotFoundError(f"File not found: {datafile}")
